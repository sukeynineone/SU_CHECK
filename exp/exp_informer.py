...
class Exp_Informer(Exp_Basic):
    ...
    def test(self, setting):
        ...
        print('mse:{}, mae:{},rmse:{},mape:{},mspe:{}'.format(mse, mae, rmse, mape, mape))
    ...
    def predict(self, args, setting, load=False):
        history_data = pd.read_csv(args.root_path + args.data_path)[args.target][-args.seq_len:].reset_index(drop=True)#只选取最后seq_len个数据点作为历史数据
        ...
        pre_data['date'] = pd.to_datetime(pre_data['date'])#把日期转换成datetime类型
        columns = ['forecast' + column for column in pre_data.columns[1:]]
        pre_data.reset_index(inplace=True, drop=True)#重置索引
        ...
        if not args.is_rolling_predict:
            if args.features == 'MS' or args.features == 'S':
                  ...
                  df.to_csv('Interval5-{}'.format(args.data_path), index=False)
            else:
            ...
            if args.features == 'MS' or args.features == 'S':
                ...
                new_df.to_csv('Interval5-{}'.format(args.data_path), index=False)
